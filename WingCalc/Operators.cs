...
internal static class Operators
{
	...
	private static readonly Dictionary<string, Operator> _operators = new List<Operator>()
	{
		...
		new("<<", (a, b) => new BinaryNode(a, b, (x, y) => Math.Truncate(x * Math.Pow(2, (int)y))), _precedenceTiers["shift"], "Converts its operands to integers and computes the left bit shift of its left-hand operator by its right-hand operator."),
		new(">>", (a, b) => new BinaryNode(a, b, (x, y) => Math.Truncate(x / Math.Pow(2, (int)y))), _precedenceTiers["shift"], "Converts its operands to integers and computes the right bit shift of its left-hand operator by its right-hand operator."),
